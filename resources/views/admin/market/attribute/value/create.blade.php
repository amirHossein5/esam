...
@section('content')
    ...
    <nav aria-label="breadcrumb">
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item font-size-12 active" aria-current="page"> ایجاد فرم برای مقادیر</li>
        </ol>
    </nav>
    ...
    <section class="row">
        <section class="col-12">
            <section class="main-body-container">
                ...
                <section>
                    <form action="{{ route('admin.market.attribute.value.store') }}" method="post" id="form">
                        @csrf
                        <section class="row">
                            <section class="col-12 ">
                                <div class="form-group">
                                    <label for="attributable_id">مقدار فرم </label>
                                    <select name="attributable_id" id="attributable_id" class="form-control form-control-sm">
                                        <option value=""> مقدار فرم را انتخاب کنید</option>
                                        @foreach ($default_values as $default_value)
                                            <option
                                                value="{{ $default_value['id'] }}"
                                                @if(old('attributable_id') == $default_value['id']) selected @endif
                                            >
                                                @if (is_string($default_value['value']))
                                                    {{ $default_value['value'] }}
                                                @endif
                                            </option>
                                        @endforeach
